...
function verificaNumero(num){
    ...
    if (num % 2 == 0){ // verificando o resto da divisão do parâmetro por 2 é 0 - no caso do -12, isso é verdadeiro
    ...
    if (num > 0){     // verificando se o número que chega por parâmetro é maior que zero, ou seja, positivo - no caso do -12, isso é falso
      str += 'positivo';
    } else {          // por ser falso, concatena a string 'negativo' na variável str. str agora é 'par, negativo'
      str += 'negativo';
    }
    
    str += ' e ';     // concatena um espaço, uma letra E e outro espaço na variável str. str agora é 'par, negativo e '
    
    if (num === parseInt(num)){ // verifica se o número que chega por parâmetro é um número inteiro - no caso do -12, é verdadeiro
...
if(isPair){ // se isPair possuir um valor truthy (true) - mesma coisa que if(isPair == true)
...
function algarismosEmUmNumero(n){
  let str = `${n}`; // transformando o número que chega em string (3.141517 -> "3.141517")
  ...
  if(n != parseInt(n)){ //se o número for diferente de ele mesmo convertido para inteiro, ou seja, se for decimal
